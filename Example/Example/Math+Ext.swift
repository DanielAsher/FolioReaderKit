...
extension CGVector {
    ...
    public static func setImpulse(from:CGPoint, to: CGPoint, force:CGFloat) -> CGVector
    {
        let dx = (to.x - from.x) * force
        let dy = (to.y - from.y) * force
...
extension CGFloat {
    ...
    public static func random(mini: CGFloat, maxi: CGFloat) -> CGFloat {
        
        let rand = CGFloat(arc4random()) / CGFloat(UINT32_MAX)
        let minimum = mini < maxi ? mini : maxi 
        return  rand * abs(mini - maxi) + minimum
    }
